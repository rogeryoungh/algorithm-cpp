...
struct NTT32OriginalRadix2AVX2 {
  ...
  NTT32OriginalRadix2AVX2(Mont32 M, u32 G) : _MX(M) {
    ...
    for (u32 i = 0; i != rank2 - 3; ++i) {
      ...
      rate4ix[i] = _MX.load(arr);
      rotate(M.mul(iprod, irt[i + 4]));
      irate4ix[i] = _MX.load(arr);
  ...
  void ntt(u32 *f, usize n) {
    ...
    for (u32 l = n / 2; l >= 8; l /= 2) {
      u32 *f0 = f, *f1 = f + l;
      for (u32 j = 0; j != l; j += 8) {
        u32x8 x = MX.load(f0 + j), y = MX.load(f1 + j);
      ...
      for (u32 i = l * 2, k = 1; i != n; i += l * 2, ++k) {
        ...
        f0 = f + i, f1 = f0 + l;
        for (u32 j = 0; j != l; j += 8) {
          u32x8 x = MX.load(f0 + j), y = MX.mul(rx, MX.load(f1 + j));
          MX.store(f0 + j, MX.add(x, y));
          MX.store(f1 + j, MX.sub(x, y));
        }
        r = M.mul(r, rate2[std::countr_one(k)]);
    ...
    for (u32 i = 0; i != n; i += 8) {
      u32x8 fi = MX.mul(rtix, MX.load(f + i)), a, b;
